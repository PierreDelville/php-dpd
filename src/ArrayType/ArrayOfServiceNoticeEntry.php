...
class ArrayOfServiceNoticeEntry extends AbstractStructArrayBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \StructType\ServiceNoticeEntry[]
     */
     ...
     * @param \StructType\ServiceNoticeEntry[] $serviceNoticeEntry
     */
     ...
     * @return \StructType\ServiceNoticeEntry[]|null
     */
    ...
    public static function validateServiceNoticeEntryForArrayConstraintsFromSetServiceNoticeEntry(array $values = array())
    {
        ...
        foreach ($values as $arrayOfServiceNoticeEntryServiceNoticeEntryItem) {
            ...
            if (!$arrayOfServiceNoticeEntryServiceNoticeEntryItem instanceof \StructType\ServiceNoticeEntry) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ServiceNoticeEntry property can only contain items of type \StructType\ServiceNoticeEntry, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \StructType\ServiceNoticeEntry[] $serviceNoticeEntry
     * @return \ArrayType\ArrayOfServiceNoticeEntry
     */
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\ServiceNoticeEntry $item
     * @return \ArrayType\ArrayOfServiceNoticeEntry
     */
    public function addToServiceNoticeEntry(\StructType\ServiceNoticeEntry $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ServiceNoticeEntry) {
            throw new \InvalidArgumentException(sprintf('The ServiceNoticeEntry property can only contain items of type \StructType\ServiceNoticeEntry, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
