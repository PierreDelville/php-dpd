...
class ArrayOfClsTraceDetails extends AbstractStructArrayBase
{
    ...
    public static function validateClsTraceDetailsForArrayConstraintsFromSetClsTraceDetails(array $values = array())
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The clsTraceDetails property can only contain items of type \StructType\ClsTraceDetails, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
    ...
    public function addToClsTraceDetails(\StructType\ClsTraceDetails $item)
    {
        ...
        if (!$item instanceof \StructType\ClsTraceDetails) {
            throw new \InvalidArgumentException(sprintf('The clsTraceDetails property can only contain items of type \StructType\ClsTraceDetails, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
